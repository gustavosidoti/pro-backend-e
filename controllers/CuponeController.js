...
export default {
    register: async(req,res) =>{
        try {
            ...
            let exist_cupone = await models.Cupone.findOne({code: data.code})
            if(exist_cupone){
                res.status(200).json({
                    message: 403,
    ...
    update: async(req,res) =>{
        try {
            ...
            let exist_cupone = await models.Cupone.findOne({code: data.code, _id:{$ne: data._id} }); // ne operador para indicar que sea diferente
            if(exist_cupone){
                res.status(200).json({
                    message: 403,
                    message_text: "EL CODIGO DEL CUPON YA EXISTE",
